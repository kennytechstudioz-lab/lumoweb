...
import { Landmark, Mail, Edit3, X, AlertCircle, Save, Plus, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function EmailsSettingsPage() {
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    try {
      if (isCreateMode) {
        await api.post('/admin/emails', form);
        showToast(`Email template "${form.name}" created successfully!`, 'success');
      } else {
        await api.put(`/admin/emails/${selectedTemplate._id}`, {
          title: form.title,
          content: form.content,
        });
        showToast(`Email template "${selectedTemplate.name}" updated successfully!`, 'success');
      }
      setIsModalOpen(false);
      fetchEmailTemplates(emailPage);
    } catch (err: any) {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= emailTotalPages) {
      fetchEmailTemplates(newPage);
    }
  };

  if (loadingEmails && emailTemplates.length === 0) {
    return (
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        {emailTemplates.length === 0 ? (
          <p className="text-slate-400 text-xs py-8 text-center font-light">No email templates seeded.</p>
        ) : (
              ...
              <table className="w-full text-left border-collapse text-xs sm:text-sm">
                <thead>
                  <tr className="text-slate-400 uppercase font-bold text-[10px] tracking-wider border-b border-slate-100 pb-3">
                    ...
                    <th className="pb-3 text-slate-400">Subject Line</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {emailTemplates.map((template, idx) => (
                    <tr key={template._id} className="hover:bg-slate-50 transition-colors">
                      <td className="py-4 font-mono font-bold text-slate-400">{(emailPage - 1) * 20 + idx + 1}</td>
                      <td className="py-4">
            ...
            {emailTotalPages > 1 && (
              <div className="flex justify-end items-center gap-2.5 pt-4 border-t border-slate-100 text-xs text-slate-600 font-mono">
                <button
                  ...
                  onClick={() => handlePageChange(emailPage - 1)}
                  className="p-1.5 border border-slate-200 rounded hover:bg-slate-50 transition-colors disabled:opacity-40 cursor-pointer"
                >
                  <ChevronLeft size={14} />
                </button>
                <span>Page <span className="font-bold text-slate-800">{emailPage}</span> of {emailTotalPages}</span>
                <button
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                    {isCreateMode ? 'Create Email Template' : `Edit Email: ${selectedTemplate?.name}`}
                  </h3>
              ...
              <div className="p-6 flex flex-col gap-5 max-h-[65vh] overflow-y-auto">
                ...
                <div className="bg-slate-50 border border-slate-200 text-slate-600 p-4 rounded-xl text-xs flex gap-2.5 items-start">
                  ...
                  <div className="flex flex-col gap-1 leading-relaxed">
                    ...
                    <span>Use double curly braces to display activity variables, e.g. <code>{"{{amount}}"}</code>, <code>{"{{currency}}"}</code>, <code>{"{{pin}}"}</code>, <code>{"{{activate}}"}</code> depending on the trigger.</span>
                  </div>
                </div>
                ...
                {isCreateMode && (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. REGISTRATION_SUCCESSFUL"
                      value={form.name}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-slate-550 uppercase">Subject Title Header</label>
                  <input
                    type="text"
                    required
                    value={form.title}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-slate-550 uppercase">Email Content Body (HTML Supported)</label>
                  <textarea
                    required
                    rows={10}
                    value={form.content}
